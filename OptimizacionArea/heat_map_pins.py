...
def heatmapcells(path, DEBUG):
    ...
    for line in file: # Loop Line by Line
        if(component_found): # If Key Word 'COMPONENT' found
            if(string2 in line): # If PLACED in Line
                counter += 1 
        ...
        if string1 in line: # If 'PINS' is on line
            component_found = True
            component += 1
            if (component == 2): # If we have seen 2 times 'PINS'
                break
    ...
    if(DEBUG):
        print('Se encontraron ', counter, ' celdas útiles.')
        print(X)
        print(Y)
